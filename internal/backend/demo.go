...
func (d *DemoBackend) FetchEmailBody(folder string, uid uint32) (*models.EmailBody, error) {
	...
	for _, e := range d.emails {
		if e.UID == uid && (folder == "" || folder == e.Folder) {
			if body, ok := demo.BodyByUID(uid); ok {
				d.bodyCache[e.MessageID] = body.TextPlain
				return body, nil
			}
		}
	}
	return &models.EmailBody{TextPlain: "(Demo body unavailable)"}, nil
}
...
func (d *DemoBackend) SearchEmails(folder, query string, bodySearch bool) ([]*models.EmailData, error) {
	...
	for _, e := range d.emails {
		...
		bodyText := ""
		if bodySearch {
			if body, ok := demo.BodyByMessageID(e.MessageID); ok {
...
func (d *DemoBackend) SearchSemanticChunked(folder string, queryVec []float32, limit int, minScore float64) ([]*models.SemanticSearchResult, error) {
	...
	var candidates []*models.EmailData
	for _, e := range d.emails {
		if d.deletedIDs[e.MessageID] {
			continue
		}
	...
	return demo.SemanticResults(candidates, queryVec, limit, minScore), nil
}

func (d *DemoBackend) GetBodyText(messageID string) (string, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	if body := d.bodyCache[messageID]; body != "" {
		return body, nil
	}
	if body, ok := demo.BodyByMessageID(messageID); ok {
		return body.TextPlain, nil
	}
	return "", nil
}
...
func (d *DemoBackend) FetchAndCacheBody(messageID string) (*models.EmailBody, error) {
	...
	for _, e := range d.emails {
		if e.MessageID == messageID {
			if body, ok := demo.BodyByMessageID(messageID); ok {
				d.bodyCache[messageID] = body.TextPlain
				return body, nil
			}
		}
	}
...
func (d *DemoBackend) GetRecentSubjectsByContact(email string, limit int) ([]string, error) {
	return demo.RecentSubjectsByContact(email, limit), nil
}
...
func (d *DemoBackend) SearchContactsSemantic(queryVec []float32, limit int, minScore float64) ([]*models.ContactSearchResult, error) {
	return demo.ContactSemanticResults(queryVec, limit, minScore), nil
}
...
func (d *DemoBackend) GetAttachment(messageID, partPath string) (*models.Attachment, error) {
	if body, ok := demo.BodyByMessageID(messageID); ok {
		for _, attachment := range body.Attachments {
			if attachment.PartPath == partPath || attachment.Filename == partPath {
				att := attachment
				return &att, nil
			}
		}
