...
func TestDemoMCPSearchFindsCreativeCommonsImageSampler(t *testing.T) {
	...
	if !strings.Contains(body, "step 5: view inline images in full screen") {
		t.Fatalf("expected Step 5 image sampler in search response: %s", callJSON)
	}
	...
	if !strings.Contains(strings.ToLower(string(callJSON)), "step 5: view inline images in full screen") {
		t.Fatalf("expected image query to find Step 5 sampler: %s", callJSON)
	}
}
