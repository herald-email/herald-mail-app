// Package mcpserver exposes Herald email operations as MCP tools over stdio.
// It is used by `herald mcp` and the legacy herald-mcp-server wrapper.
package mcpserver
...
func Run(commandName string, args []string) error {
	...
	demoMode := fs.Bool("demo", false, "Serve deterministic synthetic demo data without loading config or cache")
	...
	if *demoMode {
		if err := server.ServeStdio(newDemoMCPServer()); err != nil {
			return fmt.Errorf("MCP demo server error: %w", err)
		}
		return nil
	}
