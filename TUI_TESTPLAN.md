...
6. Repeat with the repo custom ttyd harness: `PORT=7682 EVIDENCE_DIR=reports/ttyd-image-preview tools/ttyd-image-harness/probe.sh`.
7. Optionally repeat in stock ttyd and confirm the browser flow reaches full-screen; stock ttyd can paint iTerm2 images but may miss or relocate images compared with the custom harness.
...
- Custom ttyd + xterm image-addon mode reproduces browser-visible iTerm2 OSC 1337 image behavior and the probe records screenshot plus pixel metrics for color-chart cells and large photo regions.
- Stock ttyd is only a smoke lane for browser-visible image bytes; its placement is not authoritative for acceptance.
