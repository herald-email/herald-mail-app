...
The account config selects a theme at `theme.name` and may include `theme.overrides` keyed by semantic role IDs such as `chrome.status_bar` or `focus.selection_active`. `internal/config` preserves the YAML shape, while `internal/app` resolves built-ins, installed local YAML themes from `~/.herald/themes`, and per-config overrides into a model-owned `Theme`. Missing config defaults to `inherited`, `adaptive` aliases to `inherited`, and `legacy-dark` aliases to `herald-dark`; invalid installed themes fall back to inherited with bounded status/log feedback instead of crashing startup.
