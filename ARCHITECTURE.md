...
| `internal/ai` | AI provider clients for Ollama, Claude, and OpenAI-compatible chat/embeddings, plus composite chat+embedding wiring |
...
The account config selects a keyboard profile at `keyboard.profile` (`default`, `vim`, `emacs`, or `custom`) and optionally points `keyboard.custom_keymap` at a separate YAML file. Custom keymaps extend a built-in profile, map keys only to predefined command IDs, and can opt Compose into `insert`, `normal`, or `visual` default field modes. The Default profile keeps text fields insert-first and prefers GUI-mail-style commands (`Ctrl+N`, `r`/`Ctrl+R`, `R`/`Ctrl+Shift+R`, `Ctrl+F`, `Delete`, `Shift+Delete`, `a`/`e`, `/`/`Ctrl+K`, `Alt+R`, and `F6`/`Shift+F6`) while preserving terminal aliases that do not conflict with mail-style semantics; `Alt+R` is the advertised refresh key so Timeline `Ctrl+R` remains reply-sender, while `Ctrl+R` can remain a refresh fallback only outside Timeline mail contexts. Vim preserves the terminal-oriented navigation feel (`c/f/d/D/a`, `h/j/k/l`, and modal Compose field adapters) while using the same `r` sender / `R` all reply convention, and Emacs remains insert-first with Emacs-style movement aliases.
...
**Deletion worker**
`DeletionRequest` values are sent to a buffered channel. The `deletionWorker` runs mutations through source-scoped serial lanes so one mail source is not mutated concurrently, while different sources can progress independently. User-selected deletes in the same source and folder are batched into one request; IMAP-backed sources prefer a UID-set move/copy-to-Trash → mark Deleted → expunge → remove-from-cache flow, with per-message fallback when refs are missing fresh UID identity. Results flow back via `deletionResultCh`, and Timeline progress remains based on affected message count rather than batch count. The UI updates immediately on result without waiting for a full reload.
...
- [x] `LocalBackend` exposes read-only memory search and reply-prep methods that refresh once from cached Inbox and Sent rows, skip existing immutable records, and fail quiet when memory setup is unavailable; memory search queues refresh as background AI-scheduler work while reply prep queues it as interactive memory-extraction work.
...
- [x] Compose Radar actions are explicit local commands for source inspection, phrase insertion, dismissal, resolved/saved state, and research intent; they do not send mail, write Obsidian, or call external research services.
...
- [x] Email preview renders a read-only Herald Memories thread dossier for the selected message subject, using bounded memory search and canonical vault links without mutating message, draft, or Obsidian state.
- [x] `Settings > Memories` reads aggregate memory counts from the immutable store as a bounded, read-only status check; unavailable stores never block editing configuration. The in-app panel and first-run preferences keep setup-level fields separate from advanced destinations, extraction triggers, prompt-template inventory, and thresholds so onboarding asks only for memory enablement, storage, sources, Obsidian output, and allowed memory tasks.
...
- OpenAI-compatible configs default chat to `gpt-5.4-mini` and embeddings to `text-embedding-3-small`, while preserving user-specified model names and base URLs for other vendors
...
- Setup and changed AI settings validate only the local Ollama models actually used for chat/classification or embeddings against `/api/tags` before saving, while existing configs use non-blocking warning state so offline startup remains available
...
- Semantic vectors are tied to the configured embedding provider and model, not just to the message body hash
- Cache startup records the active embedding identity as `provider:model` in SQLite metadata
- If the configured embedding provider or model changes, Herald invalidates cached email and contact embeddings before new semantic work starts
- Legacy caches without recorded embedding-model metadata are treated as compatible only with the historical local default `nomic-embed-text`; switching to another provider or model forces invalidation
