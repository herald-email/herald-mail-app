...
The TUI owns a structured, context-sensitive shortcut catalog in `internal/app` because key routing, visible focus normalization, and overlay state all live there. The bottom hint bar stays abbreviated, while the `?` help overlay renders the full catalog for the current tab, pane, overlay, and Compose mode in a compact centered modal over the current view; editable text-entry surfaces, including Compose fields, keep literal `?` for writing. Semantic search is reached through the Timeline/Contacts search input with a `? query` prefix, and Cleanup configuration surfaces follow the same compact modal pattern: `W` automation rules, `C` cleanup rules, `P` prompts, and dry-run previews compose over the current Herald view instead of replacing the app chrome.
