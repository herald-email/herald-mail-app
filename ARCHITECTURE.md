...
The standalone wizard reuses `internal/app.Settings` in a dedicated fullscreen shell rather than the in-app settings overlay. First-run onboarding defaults to IMAP-based paths: Standard IMAP, Gmail IMAP with an App Password, ProtonMail Bridge, Fastmail, iCloud, and Outlook presets; preset account paths pre-populate their known IMAP/SMTP host and port fields while still leaving those fields editable. Gmail OAuth is an experimental first-run branch hidden unless the process starts with `-experimental`; when selected it hands off to `OAuthWaitModel`, which uses the same centered modal treatment as the in-app overlay path. OAuth completion produces a candidate config with token data, not a saved config. First-run account setup passes the account candidate through `internal/accountcheck` immediately after the account details step, before AI, offline-cache, theme, keyboard, or signature preferences are shown. First-run back navigation treats `Esc` and boundary-crossing `Shift+Tab` as navigation commands, so they do not run required-field validation for the current screen. The final first-run preference save writes the already validated config without reconnecting. In-app account setup also validates candidate configs before config writes, backend replacement, or SMTP-client replacement. Normal startup for an already configured account does not use this gate, so cached/offline startup behavior remains available during provider outages. The in-app settings panel can still expose the full provider list and advanced Sync & Cleanup controls for existing configured users, but it renders as a compact centered modal over the current Herald view so opening `S` preserves screen context instead of replacing the app chrome. In panel mode, Settings starts at a top-level category menu (`Account setup`, `AI`, `Sync & Cleanup`, `Keyboard`, `Theme Selection`, `Theme Editor`, `Signature`); saving non-account categories writes the same YAML config shape and returns to that menu, while account saves validate before replacing the active account and show a compact error modal when either IMAP or SMTP fails. Theme Selection owns choosing the active theme and installing local YAML files, while Theme Editor owns semantic role overrides, reset controls, live preview, and save-as-new-theme. `Esc` unwinds focused filter state and category depth before exiting Settings without saving unsaved edits.
