...
The `v0.7.4-beta.1` release line adds the read-only Herald Memories workspace, Compose Radar, the Gollem-backed chat agent path, macOS preview printing, Timeline range-selection polish, AI role assignment controls, Fastmail provider fixes, and safer multi-account mail cleanup, preview read-state, unsubscribe, archive, header, and forwarded-attachment behavior. See [What's New in v0.7](/whats-new-in-v0-7/) for the release-delta checklist and screenshots; older release pages remain available under Release Archive so stable links keep working.
