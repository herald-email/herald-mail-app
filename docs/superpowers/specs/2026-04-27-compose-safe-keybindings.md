...
- [x] `Alt+1/2/3/4` switch to Timeline, Compose, Cleanup, and Contacts from anywhere in the main TUI, including Compose.
...
- [x] A centralized global command layer handles `Ctrl+C` and Alt chords before tab-specific handlers.
...
- [x] Leaving a non-empty Compose draft through `Alt+1/3/4` starts the existing draft persistence path.

## Verification

These checks define the minimum acceptance evidence for the implementation.

- [x] Unit tests cover Compose text insertion for `q` and digits, browse-mode `q` quit, Alt tab switching from Compose, Alt logs/chat/sidebar/refresh from Compose, and Timeline search text safety.
