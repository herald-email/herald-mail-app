...
- [x] Built-ins include `inherited`, `herald-dark`, `herald-light`, and the diverse catalog themes; `adaptive` maps to `inherited`, `legacy-dark` maps to `herald-dark`, and `crymsom` maps to `crimson`.
- [x] `-theme` launches one session with a built-in theme name or local theme YAML file without saving config.
...
- [x] Settings includes `Theme Selection` for active theme choice and local YAML install, plus `Theme Editor` for semantic role editing, xterm-256/hex inputs, xterm-grid and RGB color pickers, swatches, live preview, role reset, reset all, and save-as-new-theme.
...
- [x] Settings tests cover Theme Selection, Theme Editor, immediate switching state, save preservation, install errors, color picker updates, role-specific working overrides, and text-entry safety for theme fields.
- [x] Tmux captures cover inherited, Herald dark, Herald light, diverse built-ins, Settings Theme Selection, Theme Editor, color picker preview, custom override preview, and the minimum-size guard at required sizes.
- [x] Docs gallery screenshots are generated by `scripts/regenerate-theme-screenshots.sh` through `--demo -theme <name>` for Timeline and `HERALD_THEME_SCREENSHOT_VIEW=preview` for open preview, then visually inspected before publication.
