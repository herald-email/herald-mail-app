...
## Interaction
The overlay behaves like a read-only modal over the current screen. It must not mutate the underlying tab, pane, draft, search query, selected message, or confirmation state when opened or closed. On large terminals it stays visually compact and centered instead of expanding to the full viewport, with the current Herald view still visible around it.
...
- [x] At `220x50`, the help panel is capped near `88x24` and centered over the current view; at `80x24`, it shrinks to fit without overflow.
...
- [x] Unit tests cover `?` routing, close keys, context content, footer hints, modal sizing, backdrop preservation, and semantic-search migration.
- [x] Tmux captures show before/after behavior at `220x50` and `80x24`, including the centered compact panel on wide terminals.
