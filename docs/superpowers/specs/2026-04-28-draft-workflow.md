...
- [x] Individual Timeline draft rows show draft state alongside reply/thread markers, so a reply draft is visibly both a reply and a draft without consuming the classification `Tag` column.
- [x] Reply draft rows and previews use `Draft reply` language so draft and reply state are both visible at once.
- [x] Preview headers for drafts include `State: Draft - E edit draft - Ctrl+S send` or `State: Draft reply - E edit draft - Ctrl+S send`.
- [x] Draft reply previews include compact Timeline thread context so the surrounding conversation is visible before the draft body.
- [x] Draft-focused key hints prioritize `E: edit draft`, `Ctrl+S: send draft`, and `D: discard draft`.
...
- [x] Pressing `Ctrl+S` on a draft row, draft preview, or collapsed thread containing a draft sends the saved draft without switching to Compose, then removes the source draft only after send success.
...
- [x] Gmail sync maps the `X-GM-LABELS` `\Draft` label into `IsDraft`, so Gmail conversation grafts outside `[Gmail]/Drafts` still render as drafts.
- [x] Cache startup backfills existing rows in canonical draft folders to `is_draft=1`, so older caches pick up draft labels after upgrade.
...
- [x] Direct Timeline draft sending uses the same saved draft headers/body and never treats reply state as mutually exclusive with draft state.
...
- [x] Focused Go tests cover cache migration/read/write, IMAP draft flag mapping, MIME header parsing, Timeline draft labels, preview header text, `E` draft routing, direct Timeline draft send, and safe draft replacement.
