...
- [x] Replace the current top-of-inbox demo story with explicit Herald onboarding messages.
- [x] Make the onboarding sequence feel like email documentation, not a marketing page or generic synthetic inbox.
- [x] Keep demo mode offline, deterministic, fictional, and safe.
...
- [x] Add one primary welcome message and nine primary onboarding step messages to the demo Timeline.
- [x] Use Herald-branded senders for every onboarding message.
- [x] Order the messages exactly as Welcome, then Step 1 through Step 9 at the top of the demo Timeline.
- [x] Include concrete keypress instructions and a short explanation of the underlying feature in every message body.
- [x] Preserve or replace existing fixture coverage for attachments, inline images, HTML rendering, cleanup grouping, AI classification, semantic search, contacts, and MCP demo behavior.
...
- [x] Show the welcome overlay only in TUI `--demo` mode, never in normal IMAP sessions or MCP demo tools.
- [x] Explain that the mailbox is synthetic and safe, that the first Timeline email starts the onboarding guide, and that the user can dismiss the overlay to explore freely.
- [x] Dismiss the overlay with `Esc`, `Space`, or `Enter` without also opening a message, toggling selection, or recording the dismiss key in the demo keypress overlay.
- [x] Swallow other keys while the overlay is visible so accidental navigation does not change the underlying Timeline.
- [x] Keep `q` and `ctrl+c` available to quit from the overlay.
...
- [x] `Herald Attachments <attachments@herald.demo>` sends `Step 3: Open and save attachments`.
- [x] Step 3 includes at least two attachments and covers attachment markers, the preview attachment list, `[` / `]` selection, `s` save, and the save path prompt.
...
- [x] Step 9 covers Contacts, chat panel `c`, quick replies, `herald mcp --demo`, `herald ssh`, and suggested practice searches.
...
- [x] Start every onboarding step body with a one-sentence purpose for the step.
- [x] Include a `Try now` section with concrete actions the user can perform immediately.
- [x] Include a `What Herald is doing` section that explains the feature behavior in plain language.
- [x] Prefer short paragraphs and compact lists over long manual-style prose.
- [x] Use realistic demo content where a feature needs it, such as attachments on Step 3 and inline image attribution on Step 5.
- [x] Avoid private identity terms, real vendor brands, lorem ipsum, and any implication that demo mode is connected to a real mailbox.
...
- [x] Give the welcome and nine onboarding step messages the newest demo dates so Timeline sorting places them first.
- [x] Keep supporting fixture messages older than the onboarding sequence.
- [x] Use Herald-branded senders for the onboarding messages; supporting practice fixtures may be kept only when they are needed for feature coverage.
- [x] Rename supporting practice fixtures with `Example:` subjects and keep the mailbox focused by removing repetitive promo, statement, receipt, newsletter, and alert duplicates.
- [x] Ensure Cleanup has multiple messages from a sender or domain so sender/domain grouping remains meaningful.
- [x] Ensure Contacts remains populated and can open recent emails inline.
- [x] Ensure deterministic demo AI categories and semantic topic vectors still return meaningful results.
- [x] Ensure MCP demo tools read the same updated mailbox fixtures.
...
- [x] Add fixture tests that assert the welcome email and Step 1 through Step 9 exist, are ordered newest-to-oldest, and use Herald senders.
- [x] Add body-content tests for the key instructional promises: navigation, reply/Markdown, attachment saving, text selection, image preview, cleanup, AI rules, settings, and next steps.
- [x] Update existing tests that hard-code old demo subjects, especially the Creative Commons image sampler subject if Step 5 renames it.
- [x] Update `TUI_TESTPLAN.md` TC-46 to expect onboarding emails as the primary public demo context.
- [x] Run Go tests for demo fixtures, demo backend behavior, and MCP demo behavior.
- [x] Run tmux demo checks at `220x50`, `80x24`, and `50x15`, saving the report in `reports/`.
- [x] If implementation touches demo docs or media, update the relevant README/docs copy and regenerate affected demo screenshots or GIFs.
