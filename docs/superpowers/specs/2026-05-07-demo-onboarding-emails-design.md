...
Demo mode should teach Herald from inside the mailbox itself. The first visible Timeline messages should read as a numbered onboarding course from Herald, so a new user can open `--demo`, start at Step 1, and learn the core workflows without leaving the TUI.
...
- [ ] Add eight primary onboarding messages to the demo Timeline.
- [ ] Use Herald-branded senders for every onboarding message.
- [ ] Order the messages exactly from Step 1 through Step 8 at the top of the demo Timeline.
...
The sequence should be explicit and numbered. Each email should have a subject that starts with `Step N:` and a sender in the form `Herald <role> <local-part@herald.demo>`.

- [ ] `Herald Guide <guide@herald.demo>` sends `Step 1: Try this - move around your inbox`.
...
- [ ] `Herald Image Lab <images@herald.demo>` sends `Step 4: View inline images in full screen`.
- [ ] Step 4 retains the Creative Commons sampler behavior with embedded images, `z` full-screen mode, Kitty/iTerm2 rendering, safe fallback links or placeholders, and no remote image fetching.
- [ ] `Herald Cleanup Coach <cleanup@herald.demo>` sends `Step 5: Clean up senders and domains safely`.
- [ ] Step 5 covers Cleanup tab `3`, sender and domain grouping, `space` selection, delete/archive actions, unsubscribe hints, and preview-before-action behavior.
- [ ] `Herald AI Rules <rules@herald.demo>` sends `Step 6: Classify mail and dry-run rules`.
- [ ] Step 6 covers `a` classification, deterministic demo AI, semantic search with a sample query such as `? infrastructure budget risk`, cleanup rules `C`, automation rules `W`, prompts `P`, and dry-run previews.
- [ ] `Herald Settings <settings@herald.demo>` sends `Step 7: Configure accounts, AI, and signatures`.
- [ ] Step 7 covers the `S` settings overlay, provider configuration, local/Ollama/OpenAI-compatible AI settings, embedding model choice, and email signatures.
- [ ] `Herald Next Steps <next@herald.demo>` sends `Step 8: Explore contacts, chat, SSH, and MCP`.
- [ ] Step 8 covers Contacts, chat panel `c`, quick replies, `herald mcp --demo`, `herald ssh`, and suggested practice searches.
...
- [ ] Start every body with a one-sentence purpose for the step.
...
- [ ] Use realistic demo content where a feature needs it, such as attachments on Step 3 and inline image attribution on Step 4.
...
The onboarding messages should become the primary demo experience, but the demo data still needs enough structure to exercise Herald's public surfaces. Supporting messages may remain or be replaced as long as they do not compete with the Step 1-8 course at the top of Timeline.

- [ ] Give the eight onboarding messages the newest demo dates so Timeline sorting places them first.
...
- [ ] Add fixture tests that assert Step 1 through Step 8 exist, are ordered newest-to-oldest, and use Herald senders.
...
- [ ] Update existing tests that hard-code old demo subjects, especially the Creative Commons image sampler subject if Step 4 renames it.
