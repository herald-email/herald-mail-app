...
The automation rule, custom prompt, cleanup rule, and dry-run preview surfaces render as compact centered overlays launched from `Settings > Sync & Cleanup`, matching Settings `S` and shortcut help `?`. At `80x24`, each overlay must fit inside the viewport with internal truncation or scrolling where needed; at `50x15`, Herald's standard minimum-size guard remains in charge.

### Automation rule editor

The rule overlay must explain that it creates a real-time automation rule for future matching mail. It must also show a compact inventory of existing saved automation rules in the same compact centered overlay so reopening Settings answers "where did my rule go?"

### Custom prompt editor

The prompt overlay must explain that prompts are reusable AI instructions, not actions by themselves. It must show a compact inventory of saved prompts in the same compact centered overlay so reopening Settings answers "where did my prompt go?"

### Cleanup rules manager
